...
import { FileText, Search, Plus, Eye, Check, X, Clock } from "lucide-react";
import {
...
export function CotizacionesListaClient({
  cotizaciones,
}: {
  ...
  const filtrados = useMemo(() => {
    ...
    if (filtro !== "todos") items = items.filter((c) => c.estado === filtro);
    if (busqueda.trim()) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">
              Listado de cotizaciones y estados
            </p>
          </div>
        </div>
        <Link
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtrados.map((c) => {
                  const est =
                    ESTADO_BADGE[c.estado as string] ??
                    { cls: "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300", label: c.estado };
                  return (
                    <tr
                      key={c.id}
                        ...
                        <div className="flex items-center justify-center gap-1">
                          {c.estado === "pendiente" && (
                            <>
                              <button
                                onClick={() =>
                                  aprobar.execute({ id: c.id, estado: "aprobada" })
                                }
                              ...
                              <button
                                onClick={() =>
                                  rechazar.execute({ id: c.id, estado: "rechazada" })
                                }
