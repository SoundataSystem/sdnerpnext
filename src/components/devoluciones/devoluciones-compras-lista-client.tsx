...
import {
  aprobarDevolucionCompraAction,
  rechazarDevolucionCompraAction,
} from "@/lib/actions/devoluciones-actions";
import { formatGs } from "@/lib/devoluciones/calculos";
import type { DevolucionCompraDTO } from "@/lib/devoluciones/repository";
...
export function DevolucionesComprasListaClient({
  devoluciones,
}: {
  devoluciones: DevolucionCompraDTO[];
}) {
  const [busqueda, setBusqueda] = useState("");
  const [filtro, setFiltro] = useState("todos");

  const aprobar = useAction(aprobarDevolucionCompraAction, {
    onSuccess: () => toast.success("Devolución aprobada. Stock ajustado"),
    onError: (err) => toast.error(err.error.serverError ?? "Error"),
  });
  const rechazar = useAction(rechazarDevolucionCompraAction, {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">
              Devoluciones a proveedores y estados
            </p>
          </div>
        </div>
        <Link
              ...
              <tbody>
                {filtrados.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-3 py-2 font-mono text-xs font-medium text-orange-700 dark:text-orange-400">
                        {d.numero_devolucion ?? "—"}
                      </td>
                      <td className="px-3 py-2 font-mono text-xs text-zinc-600 dark:text-zinc-400">
                        {d.orden_compra_numero ?? "—"}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Link
                            href={`/devoluciones/compras/${d.id}`}
